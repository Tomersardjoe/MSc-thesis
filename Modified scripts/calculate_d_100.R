...
# Replace .'s with underscores to match naming convention
treeRt$tip.label <- gsub("[.]", "_", treeRt$tip.label)
...
mkWorker <- function(dataset) {
  ...
  calcD <- function(dataset, binvarry) {
    if(binvarry != "Id") {
      result <- eval(parse(text=paste("caper::phylo.d(data=dataset, binvar=",binvarry,", permut=100)", sep="")))
...
cat("All D-value calculations completed.\n")