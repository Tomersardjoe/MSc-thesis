...
def plot_clusters(df, centroids_df, outdir, chosen_taxids=None):
    """
    Scatter plot of fluidity vs openness with cluster assignments.
    Centroids are marked with larger black Xs.
    Selected species will be marked, if provided.
    """
    ...
    if chosen_taxids:
      chosen_subset = df[df['species_taxid'].isin(chosen_taxids)]
      plt.scatter(chosen_subset['fluidity_calc'], chosen_subset['openness'],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--species", required=False, help="Path to species_categories.csv")
    ...
    chosen_taxids = set()
    if args.species:
        chosen_df = pd.read_csv(args.species)
        # Grab second column (species_taxids)
        chosen_taxids = set(chosen_df.iloc[:,1].astype(str).tolist())
    
    clustered, centroids_df, best_k = run_clustering(merged.copy())
    plot_clusters(clustered, centroids_df, args.outdir, chosen_taxids)
